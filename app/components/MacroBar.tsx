...
export default function MacroBar({ calories, proteins, proteinsTarget, carbs, carbsTarget, fats, fatsTarget }: Props) {
  ...
  return (
    <div className="bg-[#0d1526] border border-[#1c2e4a] rounded-2xl px-5 py-5 shadow-[0_2px_16px_rgba(0,0,0,0.4)]">
      ...
      <ul className="divide-y divide-[#1c2e4a]">
        {macroRows.map((m) => (
          <li key={m.label} className="py-3.5 first:pt-1 last:pb-1">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-baseline gap-1.5">
                <span className={`text-xs font-bold tabular-nums ${m.textColor}`}>{m.value} g</span>
                <span className="text-[10px] text-[#4a5872]">/ {m.target} g</span>
              </div>
            </div>
            <div className="h-1 bg-[#07090f] rounded-full overflow-hidden border border-[#1c2e4a] mb-2">
              <div
            ...
            <div className="flex items-center justify-between">
              ...
              <p className="text-[10px] text-[#4a5872] tabular-nums">{m.pct}% de l'objectif</p>
            </div>
          </li>
        ))}
      </ul>
